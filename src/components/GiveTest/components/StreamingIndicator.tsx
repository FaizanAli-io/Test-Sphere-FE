...
export const StreamingIndicator: React.FC<StreamingIndicatorProps> = ({
  ...
  // Attach webcam stream to the hidden video element for ML processing.
  // Gate useProctoringML on `mlVideoReady` (set after loadedmetadata) so the
  // hook never receives a video element that hasn't started decoding frames.
  useEffect(() => {
    const video = mlVideoRef.current;
    if (!video || !initialStream || !initialStream.active) return;

    video.srcObject = initialStream;
    video.onloadedmetadata = () => {
      setMlVideoReady(true);
    };
    video.play().catch(() => {});
  }, [initialStream]);

  // AI Proctoring ML — runs face detection + leaky bucket + emits via socket.
  // Starts as soon as streaming is enabled; the emit inside the hook only fires
  // when socket.connected is true, so there is no risk of premature emission.
  useProctoringML({
    videoElement: mlVideoReady ? mlVideoRef.current : null,
    socket,
    studentId: userId,
    enabled: enabled && mlVideoReady,
  });
  ...
  return (
    <>
      {/* Hidden video element for ML face analysis — always present when enabled.
          Uses real dimensions offscreen so the browser decodes frames properly. */}
      <video
        ref={mlVideoRef}
        autoPlay
        playsInline
