...
export default function TeacherDashboard() {
  ...
  // 🔹 Utility functions
  const getSubmissionRate = (test: any) => {
      ...
      <div className="max-w-6xl mx-auto bg-white shadow-lg rounded-2xl p-8">
        ...
        <section className="mb-10">
          ...
          <div className="bg-gray-100 rounded-lg p-4">
            <p><span className="font-medium">Name:</span> {dashboardData.teacher?.name}</p>
            <p><span className="font-medium">Email:</span> {dashboardData.teacher?.email}</p>
            <p><span className="font-medium">ID:</span> {dashboardData.teacher?.id}</p>
          </div>
        </section>
        ...
        <section className="mb-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="bg-indigo-50 p-4 rounded-lg text-center">
              ...
              <p className="text-2xl text-indigo-700">{dashboardData.statistics?.total_classes || 0}</p>
            </div>
            <div className="bg-indigo-50 p-4 rounded-lg text-center">
              <h3 className="text-lg font-semibold">Total Students</h3>
              <p className="text-2xl text-indigo-700">{dashboardData.statistics?.total_students || 0}</p>
            </div>
            <div className="bg-indigo-50 p-4 rounded-lg text-center">
              <h3 className="text-lg font-semibold">Upcoming Tests</h3>
              <p className="text-2xl text-indigo-700">{dashboardData.statistics?.upcoming_tests || 0}</p>
            </div>
            <div className="bg-indigo-50 p-4 rounded-lg text-center">
              <h3 className="text-lg font-semibold">Completed Tests</h3>
              <p className="text-2xl text-indigo-700">{dashboardData.statistics?.completed_tests || 0}</p>
            </div>
          </div>
        </section>
        ...
        {/* Classes */}
        <section className="mb-10">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">Your Classes</h2>
          {dashboardData.classes?.length ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {dashboardData.classes.map((cls) => (
                <div key={cls.class_id} className="bg-white shadow rounded-lg p-4 border">
                  <h3 className="text-lg font-semibold text-indigo-700 mb-2">
                    {cls.class_name}
                  </h3>
                  <p><span className="font-medium">Code:</span> {cls.class_code}</p>
        ...
        {/* Upcoming Tests */}
        <section className="mb-10">
          <h2 className="text-xl font-semibold text-gray-700 mb-4">Upcoming Tests</h2>
        ...
        {/* Completed Tests */}
        <section>
          <h2 className="text-xl font-semibold text-gray-700 mb-4">Recent Completed Tests</h2>
          {dashboardData.completedTests?.length ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {dashboardData.completedTests.map((test) => (
                <div key={test.test_id} className="bg-white shadow rounded-lg p-4 border">
                  <h3 className="text-lg font-semibold text-indigo-700 mb-2">{test.title}</h3>
                  <p><span className="font-medium">Class:</span> {test.class_name}</p>
                  <p>
                    <span className="font-medium">Submission Rate:</span>{" "}
                    {test.attempted_students}/{test.total_students} ({getSubmissionRate(test)})
                  </p>
                  <p><span className="font-medium">Average Score:</span> {test.average_score || "N/A"}%</p>
                  <p><span className="font-medium">Completed On:</span> {formatDateTime(test.date)}</p>
                  <button
            ...
            <p className="text-gray-500">No completed tests in the last 30 days.</p>
          )}
        </section>
      </div>
    </div>
